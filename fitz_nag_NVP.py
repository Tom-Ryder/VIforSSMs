...
class IAF():
    ...
    def _create_flow(self, base_dist, p, kernel_len, batch_dims, target_dims, train_bool=True):
        base_sample, self.base_logprob = base_dist.slp(p)

        ts_aug_net = [self.ts_feats[:, :-1, :]]
        ...
        layer1A = tf.layers.conv1d(inputs=convnet_inp, filters=network_dims[0],
                                   kernel_size=kernel_len, strides=1, padding='valid', activation=None, trainable=train_bool)
        layer1B1 = tf.layers.dense(
            inputs=self.theta, units=self.network_dims[0], activation=None, trainable=train_bool)
        layer1B2 = tf.layers.dense(
            inputs=layer1B1, units=self.network_dims[0], activation=None, trainable=train_bool)
        layer1B = tf.layers.dense(
            inputs=layer1B2, units=self.network_dims[0], activation=None, trainable=train_bool)
        ...
        for i in range(1, self.num_layers - 1):
            layers.append(tf.layers.conv1d(
                inputs=layers[-1], filters=self.network_dims[i], kernel_size=1, strides=1, activation=tf.nn.elu, trainable=train_bool))
            layers.append(tf.layers.batch_normalization(layers[-1]))

        layers.append(tf.layers.conv1d(
            inputs=layers[-1], filters=2, kernel_size=1, strides=2, activation=None, trainable=train_bool))

        mu_temp, sigma_temp = tf.split(layers[-1], 2, axis=2)
        mu_aug = tf.reshape(
            tf.concat([tf.zeros(tf.shape(mu_temp)), mu_temp], axis=2), [p, -1])
        sigma_aug = tf.reshape(tf.concat([tf.ones(
            tf.shape(sigma_temp)), tf.nn.softplus(sigma_temp) + 1e-10], axis=2), [p, -1])
...
class Flow_Stack():
    ...
    def __init__(self, flows, kernel_len, batch_dims, target_dims, train_bool):
        ...
        for i in range(1, len(flows)):
            flows[i]._create_flow(
                flows[i - 1], p, kernel_len, batch_dims, target_dims, train_bool)
...
class VI_SSM():
    ...
    def __init__(self, obs, obs_bin, time_till, x0, theta_dist, priors, dt, T, p, kernel_len, batch_dims, network_dims, target_dims, no_flows, feat_window, learn_rate=1e-3, pre_train=True, train_paths=True):
        ...
        time_till_pad = np.reshape(np.repeat(np.arange(np.round((self.no_flows * self.kernel_len + self.flow_dims) * (
            self.dt / self.flow_dims), 1), -self.dt, -self.dt), self.flow_dims), (self.flow_dims, -1), 'F')
        self.time_till = np.reshape(np.concatenate(
            (time_till_pad, time_till), 1), -1, 'F')
        self.bin_feats = np.float32(np.concatenate(
            (np.ones(self.no_flows * self.kernel_len + self.flow_dims), np.zeros(self.target_dims * self.flow_dims)), axis=0))
    ...
    def _ELBO(self):
        obs_log_prob = tf.reduce_sum(tf.reshape(tfd.Normal(loc=self.obs_eval, scale=.1).log_prob(
            self.lf_sample[:, :, 1:]) * self.bin_feed, [self.p, -1]), 1)
        ...
        def alpha(x1, x2, theta):
            drift_vec = tf.concat([tf.reshape(tf.exp(theta[0]) * (x1 - x1 ** 3 - x2 + theta[1]), [-1, 1]),
                                   tf.reshape(theta[2]*x1 - x2 + 1.4, [-1, 1])], axis=1)
            return drift_vec
        ...
        beta_sqrt = tf.sqrt(self.dt) * tf.concat([tf.expand_dims(item, 1) for item in [
            tf.sqrt(tf.exp(self.theta_eval[3])), tf.sqrt(tf.exp(self.theta_eval[4]))]], 1)
    ...
    def build_flow(self):
        ...
        SSM = Flow_Stack(flows[:-1], self.kernel_len,
                         self.batch_dims * self.flow_dims, self.target_dims, self.train_paths)

        lf_sample_init, self.lf_log_prob = SSM.slp()
        self.lf_sample = tf.transpose(tf.reshape(
        ...
        self.t1 = AdamaxOptimizer(
            learning_rate=1e-3, beta1=0.9).minimize((self.lf_sample - tf.tile([[[0.], [0.]]], [self.p, 1, self.batch_dims + 1])) ** 2)

        self.t2 = AdamaxOptimizer(learning_rate=1e-3, beta1=0.9).minimize(
            (self.theta - tf.tile([[np.log(2.),1.,1.5,np.log(.5),np.log(.3)]], [self.p, 1])) ** 2)
        ...
        with tf.name_scope('loss'):
            ...
            # theta summaries
            theta_pos_index = [True, False, False, True, True]
        ...
        with tf.name_scope('optimize'):
            ...
            self.gradients, _ = tf.clip_by_global_norm(gradients, 2.5e11)
...
network_dims = [50] * 5
no_flows = 3
# priors = [(item, 1e-4) for item in [np.log(2.),1.,1.5,np.log(.5),np.log(.3)]]
priors = [(0., 10.)] * 5
...
# obs and theta
x0 = np.array([2., 3.])
f1 = open('dat/fitz_nag_obs_partial.txt', 'r')
f2 = open('dat/fitz_nag_obs_binary.txt', 'r')
f3 = open('dat/fitz_nag_time_till.txt', 'r')
...
# training!
# var_model.load('model_saves/fitz_nag_model_111111111%i.ckpt' % batch_dims)

# var_model.load('model_saves/fitz_nag_model_%i_freeze.ckpt'
# % batch_dims)

# var_model.load('model_saves/model_%i.ckpt' %
# target_dims)

# var_model.ELBO_estimate(1e5, 'locally_variant/ELBO_%i' % batch_dims)

var_model.save_paths('locally_variant/FHN_obs_paths.txt')
np.savetxt('locally_variant/FHN_local_post.txt',
  sess.run(theta_dist.sample([100000])))

# var_model.train(tensorboard_path='locally_variant/train/',
#                 save_path='model_saves/fitz_nag_model_111111111%i.ckpt' % batch_dims)

# var_model.save_paths('locally_variant/obs_paths.txt')
